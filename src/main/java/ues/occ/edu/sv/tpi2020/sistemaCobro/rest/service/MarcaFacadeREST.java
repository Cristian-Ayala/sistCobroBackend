...
public class MarcaFacadeREST implements Serializable {
    ...
    public Response create(String jsonString) {
        ...
        if (marcaFacade.create(new Marca(null, json.get("nombreMarca").getAsString(), json.get("activoMarca").getAsBoolean(), json.get("descripcionMarca").getAsString()))) {
    ...
    public Response edit(String jsonString) {
         ...
         System.out.println(json.get("descripcionMarca").getAsString());
        if (marcaFacade.edit(new Marca(json.get("idMarca").getAsInt(), json.get("nombreMarca").getAsString(), json.get("activoMarca").getAsBoolean(), json.get("descripcionMarca").getAsString()))) {
            return Response.status(Response.Status.OK).header("mensaje", "se creo con exito").build();
        } else {
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).header("mensaje", "Sin exito").build();
        }
